...
class Worker:
    ...
    async def run(self):
        """
        Background worker that pulls events, runs LangGraph, and updates DB.
        """
                                ...
                                try:
                                    ...
                                    if incident_data:
                                        ...
                                        async with async_session() as session:
                                            ...
                                            # Check if we should find existing? For MVP, create new.
                                            ...
                                            for idx, log in enumerate(result_state.get("investigation_log", [])):
                                                step = InvestigationStep(
                                                    incident_id=new_incident.id,
                                                    step_number=idx + 1,
                                                    agent_name="System", # Could verify which agent added it if we tracked it in state better
                                            ...
                                            for action_str in result_state.get("actions_queue", []):
                                                action = ResponseAction(
                                                    incident_id=new_incident.id,
                                                    action_type=action_str,
                                                    target=incident_data.get("attacker_ip", "unknown"),
                                                    status="executed" # Assuming executed by Response Agent in graph
                                                )
                                                session.add(action)
                                            
                                            await session.commit()
                                            logger.info(f"Persisted incident {new_incident.id} to DB")

                                    logger.info(f"Graph execution finished. Steps: {result_state.get('investigation_log')}")

                                except json.JSONDecodeError:
                                    logger.warning(f"Invalid JSON payload: {payload_str}")
